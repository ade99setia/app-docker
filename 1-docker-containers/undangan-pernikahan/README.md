...
~~~powershell
# 1. Export Volume Private ke .tar.gz (Hasilnya tidak akan 1KB lagi)
docker run --rm -v undangan-pernikahan_data_private:/from -v ${PWD}:/to alpine sh -c "cd /from && tar czf /to/private.tar.gz ."

# 2. Export Volume Uploads ke .tar.gz
docker run --rm -v undangan-pernikahan_data_uploads:/from -v ${PWD}:/to alpine sh -c "cd /from && tar czf /to/uploads.tar.gz ."
...
~~~bash
# 1. Restore Private (Musik/Gambar)
docker run --rm -v undangan-pernikahan_data_private:/to -v $(pwd):/from alpine sh -c "cd /to && tar xzf /from/private.tar.gz"

# 2. Restore Uploads
docker run --rm -v undangan-pernikahan_data_uploads:/to -v $(pwd):/from alpine sh -c "cd /to && tar xzf /from/uploads.tar.gz"
